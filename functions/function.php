...
try {
  ...
  //$pdo = new PDO('mysql:host=10.0.0.52;dbname=joomladb', 'user', 'datos@bar2021');
  $pdo = new PDO('mysql:host=localhost;dbname=pds_db', 'remote_admin', 'datos@bar2022');
...
{ 
  ...
  newsletter.id as newsletterid,
  newsletter.author as newsletterauthor,
  newsletter.issue_month as isyow_id,
  newsletter.dateUploaded as news_dateUploaded,
  newsletter.date_letter as news_date_letter,
...
{ 
  ...
  attachments.size as fileSize,
  attachments.id as fileID,
  attachments.fileExtension as fileExtension FROM newsletter INNER JOIN attachments ON attachments.id=newsletter.attachment WHERE newsletter.id != :newsLetter and newsletter.issue_month = :isyu_id";
  $stmt = $pdo->prepare($sql);
  $stmt->execute(array(
...
//END VIEWING OF OTHER ARTICLE WITHIN THE ISSUE
